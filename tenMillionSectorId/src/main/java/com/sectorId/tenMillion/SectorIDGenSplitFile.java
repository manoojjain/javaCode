...
public class SectorIDGenSplitFile {
    ...
    public static void main(String[] args) throws Exception {
            ...
            int sectotIdLinesInSingleFile = 100000;
            int fileName = 1;
            ...
            for(int i=0, j=0; i<totalSectIdLines; i++,j++)
            {  
                if( j == sectotIdLinesInSingleFile) {
                    j=0;
                    fileName++;
                    sectorIdFilePath = path+fileName;
                    sectorIdFile = checkFilePresent(sectorIdFilePath);
                    bwForSectorIdFile = new BufferedWriter(new FileWriter(sectorIdFilePath, true));
                }
                ...
                while (lineTokenizer.hasMoreTokens()) {
                    ...
                    if (a == 1) {
                        bwForMetaDataFile.write(sectorId+" "+fileName+" "+(j+1));
